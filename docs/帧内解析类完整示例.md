# 帧内解析类（ThinkGear Payload / TLV）— 完整示例代码

> 本文**仅文档**：可复制到工程中的 `thinkgear/` 或 `protocol/` 目录，并在 **CMake** 中加入对应 `.h/.cpp`。  
> **前置**：`ThinkGearFrameAssembler` 已发出 **完整帧** `[0xAA][0xAA][N][Payload×N][Checksum]`。本类只解析 **Payload** 内的代码字与数据。
...
| **`0x80` 的符号** | 按 **无符号 16 位拼出再转 `qint16`**；若设备定义为偏移二进制，可在槽里再减偏置。 |
